...
| A列 (項目) | B列 (値) | 説明 |
|------------|---------|------|
| 土壌湿度閾値 | `0.4` | この値以下で給水 (0.0=乾燥, 1.0=湿潤) |
| 給水時間(秒) | `10` | ポンプ稼働時間 |
| スケジュール時刻 | `07:00` | カンマ区切りで複数可 |
| 給水モード | `AUTO` | AUTO / MANUAL / OFF |
| 手動給水 | `FALSE` | TRUE にすると即時給水 |
| 通知 | `TRUE` | 通知の有効/無効 |
| センサ1乾燥値 | `442` | センサ1を乾燥させた時の生値 |
| センサ1湿潤値 | `297` | センサ1を湿らせた時の生値 |
| センサ2乾燥値 | `5` | センサ2を乾燥させた時の生値 |
| センサ2湿潤値 | `47` | センサ2を湿らせた時の生値 |

> **キャリブレーション**: 2つの土壌センサーは特性が異なる場合があります。乾燥時と湿潤時の実測値を記録することで、正規化された湿度(0.0〜1.0)で統一的に判定できます。乾燥値>湿潤値でも、その逆でも正しく動作します。
